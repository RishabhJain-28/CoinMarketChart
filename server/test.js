...
async function a() {
  ...
  for (var i = 0; i < l; i++) {
    ...
    // console.log(start.format("MMMM Do YYYY, h:mm:ss a"));
    const formatedDate = currentDate.format("YYYY-MM-D").split("-");
    const year_val = parseInt(formatedDate[0]);
    const month_val = parseInt(formatedDate[1]);
    const day_val = parseInt(formatedDate[2]);
    let bucket = await Prices.findOne({
      "date.year": year_val,
      "date.month": month_val,
      "date.day": day_val,
    });
    if (!bucket) {
      bucket = new Prices({
